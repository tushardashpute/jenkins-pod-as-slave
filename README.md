...
In the cluster, create a Namespace and ServiceAccount which will be used by Jenkins for authorization.
   ...
   [root@ip-172-31-13-254 opt]# aws eks describe-cluster --name my-cluster --region us-east-2
   {
       "cluster": {
           "status": "ACTIVE", 
   ...
   [root@ip-172-31-13-254 opt]# k get secrets -n test
   NAME                                         TYPE                                  DATA   AGE
   ...
   [root@ip-172-31-13-254 opt]# k get secrets -n test jenkins-slaves-service-account-token-z5dpt -o jsonpath='{.data.token}' | base64 --decode
